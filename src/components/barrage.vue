<!--
 * @Author: your name
 * @Date: 2021-01-29 17:03:09
 * @LastEditTime: 2021-01-29 17:14:53
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \vue-h5-template\src\components\barrage.vue
-->
<!--
 * @Author: your name
 * @Date: 2020-03-18 13:26:53
 * @LastEditTime: 2021-01-29 11:04:25
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
...
<template>
  <div class="z_barrage-container">
    <canvas ref="canvasContainer" :width="containerWidth" :height="containerHeight" style="display: none;" />
...
export default {
  ...
  mounted() {
    ...
    this.containerHeight = window.screen.height * this.screenPercent > 2 * this.barrageHeight ? window.screen.height *
        this.screenPercent : (this.barrageHeight + 30) // 设定总高度
  ...
  methods: {
        ...
        for (let j = 0; j < this.channelsArray[i].length; j++) {
          try {
            ...
            if (barrage.x <= this.containerWidth) { // 弹幕显示
              this.borderColor && this.drawRoundRectBorder(this.ctx, barrage.x - this.barrageHeight / 2, i * (this
                .barrageHeight + 20) + 20, barrage.width + this.barrageHeight, this.barrageHeight, this
                .barrageHeight / 2)
              this.drawRoundRect(this.ctx, barrage.bgColor, barrage.x - this.barrageHeight / 2, i * (this
                .barrageHeight + 20) + 21, barrage.width + this.barrageHeight, this.barrageHeight - 2, this
                .barrageHeight / 2)
              this.ctx.fillStyle = `${barrage.color}`
              this.ctx.fillText(barrage.content, barrage.x + (barrage.icon ? this.barrageHeight / 2 + 20 : -5), i * (
                this.barrageHeight + 20) + this.barrageHeight)
              if (barrage.icon) {
                this.circleImg(this.ctx, barrage.icon, barrage.x - 10, i * (this.barrageHeight + 20) + 26, 24)
              }
              if (barrage.tagImage) {
                this.originImg(this.ctx, barrage.tagImage, barrage.x - this.barrageHeight - 10, i * (this
                  .barrageHeight + 20) + 20, this.barrageHeight, this.barrageHeight)
              }
            }
            ...
            // 弹幕插入时机判断
            if (barrage.x <= Math.floor((this.containerWidth - barrage.width - 40)) && barrage.x >= Math.floor(this
              .containerWidth - barrage.width - 40 - this.speed) && (j === this.channelsArray[i].length - 1) && this
              .barrageArray.length !== 0) {
              const item = this.barrageArray.shift()
    ...
    barrageClickEvent() {
      document.getElementById('canvas').addEventListener('click', (e) => {
        ...
        const channelIndex = Math.floor(p.y / (this.barrageHeight + 30))
